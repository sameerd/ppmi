...
event.ids[as.character(event.ids$CODE) %in% levels(merged.updrs$EVENT_ID),
	  c("CODE", "DECODE")]
@
...
<<plot-pd-part3-dist, cache=TRUE, echo=TRUE, fig.width=8, fig.height=5>>=
...
pd.pats.sp <- lapply(pd.pats.sp, function(z) {
	bl <- z[z$EVENT_ID == "BL", "PART3SCORE"]
	z$PART3MINUSBL <- z$PART3SCORE - as.numeric(bl)
	z$SCOREDIFF_CAT <- paste0(as.character(z$EVENT_ID), "-BL")
	z
})
pd.pats.short <- do.call(rbind.data.frame, pd.pats.sp)
pd.pats.short$SCOREDIFF_CAT<- as.factor(pd.pats.short$SCOREDIFF_CAT)

#number of patients in each score difference category
summary(pd.pats.short$SCOREDIFF_CAT)

qplot(SCOREDIFF_CAT, PART3MINUSBL, data=pd.pats.short, geom="boxplot",
...
pd.pats.short.ts <- pd.pats.short
pd.pats.short.ts$PATNO <- as.factor(pd.pats.short.ts$PATNO)

desired.events <- c("BL", sprintf("V%02d", 1:12))

pd.pats.short.ts <- pd.pats.short.ts[
			pd.pats.short.ts$EVENT_ID %in% desired.events,]
pd.pats.short.ts <- droplevels(pd.pats.short.ts)

ggplot(data=pd.pats.short.ts, aes(x=SCOREDIFF_CAT, PART3MINUSBL, group=PATNO)) + 
...
for (i in 1:length(part.3.variables)) {
  ...
  gg <- ggplot(data=pd.pats.short.ts, 
               aes_string(x="EVENT_ID", y=currvar, group="PATNO")) + 
...
\noindent
Lets take a look at which patients are outliers

<<outliers-pd-part3-ts, cache=TRUE, echo=TRUE>>=
pd.pats.short.split <- split(pd.pats.short.ts, pd.pats.short.ts$SCOREDIFF_CAT)
pd.pats.short.split[["BL-BL"]] <- NULL

#Patient Numbers that are outliers at each level
sapply(pd.pats.short.split, function(z) {
  maxes <- as.numeric(as.character(z$PATNO[which.max(z$PART3MINUSBL)]))
  mins <- as.numeric(as.character(z$PATNO[which.min(z$PART3MINUSBL)]))
...
pd.pats.meds <- merge(pd.pats.short, meds.dat[,
		      c("PATNO", "EVENT_ID", "PDMEDYN", "ONLDOPA", "ONDOPAG", 
			"ONOTHER", "FULNUPDR", "PDMEDDT", "PDMEDTM", "NUPDRTM")])

table(pd.pats.meds$EVENT_ID, pd.pats.meds$FULNUPDR)
@
